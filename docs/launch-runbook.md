...
## Notes / gotchas
- Homepage content comes from Sanity (project `cpkoqe2k`, already seeded). Stories/Appeals/Events render from the in-code seed unless/until staff add them in Studio (read layer prefers Sanity, falls back to seed). No content blocker for launch.
